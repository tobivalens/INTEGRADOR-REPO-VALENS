...
public class AdminController {
    ...
    @GetMapping("/doctors/checkCedula")
    public ResponseEntity<?> checkCedula(@RequestParam String cedula) {
        boolean exists = doctorRepository.existsByCedula(cedula);
        ...
        if (authorization.startsWith("Bearer ")) {
            ...
            try {
                ...
                if (doctor.getName() == null || doctor.getLastname() == null || doctor.getEmail() == null ||
                        doctor.getCedula() == null || doctor.getPhone() == null || doctor.getPassword() == null) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(Map.of("message", "Todos los campos son obligatorios"));
                }

                if (!doctor.getCedula().matches("\\d+")) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(Map.of("message", "La cédula debe contener solo números"));
                }

                if (doctorRepository.existsByCedula(doctor.getCedula())) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(Map.of("message", "Ya existe un doctor con la misma cédula"));
                }
                var hashedPassword = PasswordHasher.hashPassword(doctor.getPassword());
                doctor.setPassword(hashedPassword);

                doctorRepository.save(doctor);
                return ResponseEntity.status(HttpStatus.OK)
                        .body(Map.of("message", "Doctor agregado exitosamente"));
            } catch (Exception ex) {
